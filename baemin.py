...
def init_db():
    try:
        conn = pymysql.connect(
            host='172.30.1.12',
            user='root',          # 경훈님이 바꾼 계정명으로 수정 (예: team_user)
            password='1234',      # 설정한 비밀번호
            db='baemin',              # 데이터베이스 이름
            port=3306,                # 로컬 MariaDB 기본 포트
            charset='utf8mb4',
...
if conn:
    ...
    with col_menu:
        ...
        stores_df = pd.read_sql("SELECT id, name FROM stores ORDER BY id DESC", conn)
        
        if not stores_df.empty:
            with st.form("menu_form", clear_on_submit=True):
                target_store = st.selectbox("가게 선택", stores_df['name'])
                target_id = stores_df[stores_df['name'] == target_store]['id'].values[0]
            ...
            st.divider()
            menu_view = pd.read_sql(f"SELECT menu_name, price FROM menus WHERE store_id = {target_id}", conn)
            st.write(f"🔍 {target_store} 메뉴 목록")
    ...
    if st.checkbox("전체 저장 데이터 보기"):
        ...
        all_data = pd.read_sql("""
            SELECT s.name as 가게명, s.category as 카테고리, s.rating as 별점, 
